...
public class VideoActivity extends Activity implements OnSeekBarChangeListener, Callback, OnPreparedListener, OnCompletionListener, OnBufferingUpdateListener,
...
//		player.setDisplay(holder);
	...
	private void playVideo() {
		if (extras.getString("file_name").equals("VIDEO_URI")) {
			showToast("Please, set the video URI in HelloAndroidActivity.java in onClick(View v) method");
		} else {
			new Thread(new Runnable() {
				public void run() {
					try {
						player.setDataSource(extras.getString("file_name"));
						player.prepare();
	...
	private void showToast(final String string) {
		runOnUiThread(new Runnable() {
			public void run() {
				Toast.makeText(VideoActivity.this, string, Toast.LENGTH_LONG).show();
				finish();
			}
		});
	}
	...
	public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
		...
		if (!fromUser) {
			textViewPlayed.setText(durationInSecondsToString(progress));
		}
	}
	...
	public String durationInSecondsToString(int sec){
		int hours = sec / 3600; 
